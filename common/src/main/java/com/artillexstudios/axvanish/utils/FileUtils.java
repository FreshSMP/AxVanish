...
public final class FileUtils {
    ...
    public static void copyFromResource(@NotNull String path) {
        try (ZipFile zip = new ZipFile(Paths.get(AxVanishPlugin.instance().getClass().getProtectionDomain().getCodeSource().getLocation().toURI()).toFile())) {
            for (Iterator<? extends ZipEntry> it = zip.entries().asIterator(); it.hasNext(); ) {
                ...
                InputStream resource = AxVanishPlugin.instance().getResource(entry.getName());
                if (resource == null) {
